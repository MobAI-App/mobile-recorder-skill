#!/usr/bin/env bash
# Record an iOS/Android device by consuming MobAI's HTTP MJPEG stream.
#
# Use this when:
#   - the device is iOS Simulator and `xcrun simctl io recordVideo` is broken
#     (e.g. SimRenderServer error 2 — common on macOS Sequoia + Xcode 16);
#   - the device is a physical iOS device (no native scriptable recording);
#   - any time you want native iPhone/Android pixels without recording the
#     host's screen.
#
...
#
# The script:
#   - validates that the stream URL returns frames
#   - starts ffmpeg in the background, writes <output>.pid
#   - returns immediately so the agent can run the .mob via test_run
#
# Stop with:
