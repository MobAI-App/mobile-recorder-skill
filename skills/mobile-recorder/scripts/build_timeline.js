...
function parseAction(text, lineNumber, metadata) {
  ...
  // `wait_for "Element" timeout:N` - recording-window-allowed predicate
  // that blocks until the element paints. Treated as a wait event in the
  // timeline; actual durationMs comes from test_run.json. The .mob's
  // `# Reason:` comment isn't required here (wait_for is implicitly
  // technical - sync on app state change, not for viewer pacing).
  m = text.match(/^wait_for\s+(.+?)(?:\s+timeout:(\d+))?$/i);
  if (m) {
    const target = m[1];
    const timeoutMs = m[2] ? Number(m[2]) : null;
    return {
      kind: "wait_for",
      lineNumber,
      text,
...
function buildTimeline(mobPath, testRunPath, outPath, options) {
  ...
  for (const action of actions) {
    ...
    if (action.kind === "tap") {
      ...
      // wait_for inside the recording window blocks until the predicate
      // matches. The actual elapsed time comes from test_run.json (the
      // `stepDuration` resolved earlier). We emit a wait event so the
      // timeline accounts for the gap.
      events.push({
        timeMs,
        type: "wait",
        line: action.lineNumber,
