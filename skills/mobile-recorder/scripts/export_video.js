...
function writeCaptionsSidecar({ srcStart, dstStart, warp }) {
  ...
  // Fallback: build directly from editor.captions[] (or, via
  // resolveCaptions's own fallback, from .mob # Caption: comments).
  // Captions live in choreography ms, but the exported video is in trimmed
  // post-warp seconds. The same chain the propagated sidecar already went
  // through must be replayed here: choreography ms → raw-recording seconds
  // (add recordStartOffsetMs) → warp to post-speed seconds (if a warp is
  // loaded) → subtract dstStart. Without the warp step, captions in this
  // branch drift relative to the trim points.
  ...
  const captions = ctx.resolveCaptions().map((c) => ({
    startMs: choreoMsToOutputMs(c.startMs),
    endMs:   choreoMsToOutputMs(c.endMs),
    text:    c.text,
  })).filter((c) => c.endMs > 0);
