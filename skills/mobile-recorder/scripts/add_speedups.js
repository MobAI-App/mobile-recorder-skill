...
const probe = spawnSync("ffprobe", [
  ...
  // `default` with nokey=1 returns just "<value>\n". Using csv=p=0 here
  // would append a trailing comma even for a single field, which makes
  // Number("50.633333,") → NaN.
  "-of", "default=noprint_wrappers=1:nokey=1", INPUT,
]);
if (probe.status !== 0) { console.error("ffprobe failed:", probe.stderr.toString()); process.exit(4); }
const srcDuration = parseFloat(probe.stdout.toString().trim());
if (!(srcDuration > 0)) {
  console.error(`could not determine source duration for ${INPUT} (ffprobe returned ${JSON.stringify(probe.stdout.toString())})`);
  process.exit(4);
}

// Editor spans are choreography-ms; the input mp4 lives in raw-recording
// seconds. Add recordStartOffsetMs to translate.
...
for (const d of ctx.editor.directives) {
  ...
  if (!(d.factor > 0) || d.factor === 1) {
    console.error(`speed directive: factor must be > 0 and != 1 (got ${d.factor})`);
    process.exit(2);
  }
  // Two anchor shapes (delays apply to both):
  //   {line, side, startDelayMs?, endDelayMs?}
  //   {fromLine, toLine, startDelayMs?, endDelayMs?}
  let span, label;
  if (d.fromLine != null && d.toLine != null) {
    span = ctx.resolveLineRange(d);
    label = `fromLine ${d.fromLine} → toLine ${d.toLine}`;
  } else {
    if (d.line == null || d.side == null) {
      console.error(`speed directive missing either {line,side} or {fromLine,toLine}: ${JSON.stringify(d)}`);
      process.exit(2);
    }
    span = ctx.resolveSpan(d);
    label = `line ${d.line} ${d.side}`;
  }
  const { srcStart, srcEnd } = spanToSrcSeconds(span);
  if (srcEnd <= srcStart) {
    console.warn(`speed directive on ${label} has zero duration; skipping`);
    continue;
  }
  specs.push({ srcStart, srcEnd, factor: d.factor, label });
}
