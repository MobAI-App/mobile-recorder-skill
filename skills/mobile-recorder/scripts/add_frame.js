...
// Phone proportions follow real iPhone: ~3% bezel of width, ~12% corner
// radius. The dependency is circular (screen.w depends on bezel_t which
// depends on screen.w), so we estimate phone width with bezel=0 first,
// derive proportional defaults, then refit.
...
// Bezel covers the rectangular recording's corners with its rounded
// outer ring, so BEZEL_T must be at least ~0.42 × R - otherwise the
// outer rounded corner can't reach pixels at (screen.x, screen.y) and
// the recording's corner leaks. The alphamerge mask below relaxes that
// constraint by giving the recording itself a rounded shape.
...
// Two-pass composite (one ~1s pre-bake of bg + bezel into a template PNG,
// then per-frame scale + alphamerge + overlay). The single-chain version
// with three looped PNG inputs hit pathological framesync slowness on
// macOS - 8+ minutes for a 30s clip; the split runs in seconds.
...
// alphamerge runs on the small scaled recording, not the canvas - the
// 4-input version triggered pathological framesync slowness on macOS.
const args = [
  "-y",
  "-i", INPUT,
