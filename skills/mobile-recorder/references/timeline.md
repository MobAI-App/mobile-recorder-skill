...
type DemoTimelineEvent = {
  ...
  type:
    ...
    | "wait"                      // `delay N` OR `wait_for "Element" timeout:N`
  ...
  caption?: string                // from "# Caption: ..." comment (legacy -
                                  // editor.captions[] is preferred; this
                                  // remains as a fallback when the array
                                  // is absent)
  ...
  durationMs?: number             // wait, swipe, AND tap duration. For taps
                                  ...
                                  // For wait_for, the actual elapsed ms
                                  // until the predicate matched (from
                                  // test_run.json).
  reason?: "technical" | "viewer_readability"   // from "# Reason: ..." comment
                                  // (wait_for events default to "technical"
                                  // - no comment required)

  path?: string                   // screenshot output path
  key?: string                    // press_key argument (home|back|enter)
  target?: string                 // navigate target (home), OR the
                                  // predicate text on a wait_for event
                                  // (e.g. `"@interlap"`)
